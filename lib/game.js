(function(root) {
  ...
  Game.prototype.addNewLine = function() {
    ...
    for (var i = 0; i < Game.xDimChars; i++) {
      ...
      var isEnterable = this.checkConflicts(gridPos, letter);
      if (!isEnterable) {
        letter = Game.LETTERS[Math.floor(Math.random() * Game.LETTERS.length)];
        isEnterable = this.checkConflicts(gridPos, letter);
      }
      var newTile = new LetterFall.Tile({
        vel: [0, this.speed],
        letter: isEnterable ? letter : " ",
        drawPos: drawPos,
        game: this,
  ...
  Game.prototype.bindResetHandler = function() {
    $(document).keypress(function(e) {
      if (e.charCode === 32 && newGame.isGameOver) {
